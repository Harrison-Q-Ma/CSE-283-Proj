...
# ── 1) Load TPM matrix ────────────────────────────
tpm <- read.delim("~/Desktop/BISB program/25SP/BENG 203:CSE 283/CSE-283-Proj/1_bc.classifier/output/1_QC/pnas_normal_tpm_normalized_filtered.csv",
...
hvg_stats <- tpm %>%
  ...
  select(X, avg_expr, var_expr, cv2)
...
hvg_ranked$category<-case_when(hvg_ranked$cv2>=7.036656 ~ "Highly Variable Genes",
                               TRUE ~ "other genes")
